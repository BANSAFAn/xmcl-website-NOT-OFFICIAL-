...
import { Download, HardDrive, Gamepad2, Zap, Sparkles, ExternalLink, ArrowRight, Star, Layers } from 'lucide-react';
  ...
  const features = useMemo(() => [
    {
      ...
      links: [
        { name: 'CurseForge', url: 'https://www.curseforge.com/minecraft/mc-mods', color: 'orange' },
    ...
    },
    {
      ...
      links: [
        { name: t('home.hardLink'), url: 'https://en.wikipedia.org/wiki/Hard_link', color: 'cyan' },
        { name: t('home.symbolicLink'), url: 'https://en.wikipedia.org/wiki/Symbolic_link', color: 'indigo' }
      ]
    },
    {
      icon: Gamepad2,
      ...
      links: [
        { name: 'Forge', url: 'https://files.minecraftforge.net/', color: 'red' },
    ...
    },
    {
      ...
      links: [
        { name: t('home.p2p'), url: 'https://en.wikipedia.org/wiki/Peer-to-peer', color: 'purple' }
      ]
    }
  ], [t]);
...
const FloatingOrbs = memo(() => {
  const orbs = useMemo(() => [
    { size: 500, color: 'bg-purple-500/20', style: { left: '10%', top: '10%' }, duration: 20 },
    { size: 400, color: 'bg-blue-500/20', style: { right: '15%', top: '20%' }, duration: 25 },
    { size: 450, color: 'bg-cyan-500/20', style: { left: '20%', bottom: '15%' }, duration: 22 },
    { size: 350, color: 'bg-pink-500/20', style: { right: '25%', bottom: '20%' }, duration: 18 },
  ], []);
...
const FeatureCard = memo(({ feature, index, scrollProgress }) => {
  ...
  const rotateX = useSpring(useTransform(mouseY, [-0.5, 0.5], [5, -5]), ANIMATION_CONFIG.spring);
  const rotateY = useSpring(useTransform(mouseX, [-0.5, 0.5], [-5, 5]), ANIMATION_CONFIG.spring);
...
const ImageSection = memo(({ cardRef, feature, reverse, isHovered, rotateX, rotateY }) => (
  <motion.div
    ref={cardRef}
    ...
    <div className="relative group">
      ...
      <div className="relative rounded-3xl bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900 p-1 shadow-2xl">
        <div className="relative overflow-hidden rounded-[1.3rem] bg-white dark:bg-slate-950">
          <div className="absolute inset-0 opacity-10 dark:opacity-5 pointer-events-none z-20" style={{
          ...
          <div className={`absolute inset-0 bg-gradient-to-br ${feature.gradient} opacity-0 group-hover:opacity-20 transition-opacity duration-500 mix-blend-overlay pointer-events-none z-20`} />
        </div>
      </div>
      ...
      >
        <feature.icon className="w-8 h-8 text-white drop-shadow-lg" strokeWidth={2.5} />
      </motion.div>
...
const ContentSection = memo(({ feature, reverse, index }) => (
  ...
  >
    <div className="space-y-4">
      ...
      >
        <div className={`w-1.5 h-12 bg-gradient-to-b ${feature.gradient} rounded-full`} />
...
const ActionButton = memo(({ link, index }) => (
  <motion.a
    ...
    whileInView={{ opacity: 1, scale: 1 }}
    viewport={{ once: true }}
    transition={{ delay: 0.8 + index * 0.1, type: "spring", ...ANIMATION_CONFIG.buttonSpring }}
